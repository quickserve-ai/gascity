...
// it reads the bead, merges kvs into the document it read (untouched keys keep
// their stored JSON, see metadataRawWithOverrides), and writes the merged
// document back only while the bead still carries the row version that read
// returned. A retry must call this whole operation again so metadata committed
// by the competing writer is merged rather than overwritten from a stale read.
