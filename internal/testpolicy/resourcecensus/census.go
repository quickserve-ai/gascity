...
var bootstrapPolicy = Ledger{
	...
	Medium: []MediumOwner{
		...
		},
		{
			...
			Owner:           "TestKillSocketRootServersReapsSpawnedServer",
			Resources:       []Resource{ResourceTmux},
			OwnerBead:       "ga-utvl",
			Invariant:       "the KillSocketRootServers reaping proof is a checked Medium tmux owner",
			ResourceOwner:   "the probe server is spawned on this test's private /tmp socket root and killed by kill-server in its own cleanup",
			MigrationTarget: "P0.4c-tmux",
			Expires:         "2026-10-01",
		},
		{
			PackageDir:      "test/tmuxtest",
			PackageName:     "tmuxtest",
			Owner:           "TestSweepOrphanPIDPrefixedDirsReapsServerBeforeRemovingParent",
			Resources:       []Resource{ResourceTmux},
			OwnerBead:       "ga-3qlrnv",
			Invariant:       "the orphan-sweep reaping proof is a checked Medium tmux owner",
			ResourceOwner:   "the probe server is spawned on this test's private /tmp socket root and killed by PID in its own cleanup, because the test deletes the socket dir",
			MigrationTarget: "P0.4c-tmux",
			Expires:         "2026-10-01",
		},
		{
