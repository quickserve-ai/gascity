...
type SessionTerminatedPayload struct {
	...
	// CountsNumerator/CountsDenominator are written out rather than recomputed
	// by readers. The bucket rules are a JUDGEMENT (handoff-target reports on
	// its own line; observed-dead is out of the denominator), and a judgement
	// re-derived independently by every consumer drifts. Recording the decision
	// makes a later change to it visible as a change in the data.
