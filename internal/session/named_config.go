...
// while the identity stayed unresolvable). The bead-tier matcher
// (BeadConflictsWithNamedSession) still refuses this shape for RESOLUTION:
// mail/nudge must never bind to an unstamped shadow.
func InfoIsAdoptablePoolShadow(i Info, spec NamedSessionSpec) bool {
	...
	// Adoption relies on the builder's identity-keyed session-name binding
	// (resolveSessionName looks the IDENTITY up against agent_name/template)
	// actually finding this shadow. When the named session's identity differs
	// from its backing template, that lookup misses the shadow, the desired
	// entry lands on the canonical name, and the create path blocks on the
	// held alias — suppressing the conflict there would trade a loud wedge
	// for a silent one. Only the identity-named shape is adoptable.
...
// configured named session. DELIBERATE divergence from the bead tier
...
// refusing that shape so resolution never binds to it unstamped.
