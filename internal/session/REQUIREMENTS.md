...
| SESSION-RUNTIME-010 | Current-plus-unreused-history writer identity set | Liveness writers (the ga-56nq1a lease heartbeat) authorize per-bead writes under the identities the session answers to NOW — session bead ID, raw session_name, configured_named_identity, current alias (`CurrentAssigneeIdentities`) — PLUS each `alias_history` entry that no OTHER live session currently answers to (`cmd/gc` `hookHeartbeatUnreusedPriorAliases`, one live `ResolveID` per prior alias; ambiguous or failed lookups exclude the alias, and every exclusion is diagnosed). A rebranded session keeps the work assigned under its old alias, which orphan release also attributes to it, so a current-only writer set let that work's lease expire under a live owner. A prior alias a later LIVE session has taken stays excluded: vouching through it would keep the successor's claims looking alive after the successor dies. Readers deciding whether to strip work keep the full historical set (`AssigneeIdentities`). | `internal/session/assignee_identities_test.go` `TestCurrentAssigneeIdentitiesExcludesAliasHistory`; `cmd/gc/cmd_hook_heartbeat_test.go` `TestHookHeartbeatUnreusedPriorAliasesVouchesOnlyThroughUnclaimedHistory`, `TestHookHeartbeatIdentitiesUnionsCurrentAndUnreusedHistory`; gascity PR #128 (codex round-2 P1, round-8 P1-b); ga-56nq1a |
| SESSION-RUNTIME-011 | Partial list results still heartbeat | When the store's list returns rows together with a `PartialResultError` (one tier failed after the other matched, or one entry failed to parse), the heartbeat refreshes every returned row and counts the partial failure as one refusal; it never discards healthy claims over a malformed neighbour. A total list failure yields no beats. | `cmd/gc/cmd_hook_heartbeat_test.go` `TestCmdHookHeartbeatBeatsRowsReturnedBesideAPartialListError`; `cmd/gc/cmd_hook_heartbeat.go`; gascity PR #128 (codex round-3 P1); ga-56nq1a |
| SESSION-RUNTIME-012 | Heartbeat seam coverage is per overlay, not universal | The turn-driven refresher rides `gc mail check --inject` (start of turn). Overlays that traverse it: claude, codex, cursor, copilot, gemini, antigravity, kiro, opencode, mimocode, pi, omp. The kimi overlay registers only a `SessionStart` hook, so an armed kimi seat receives no turn-driven refresh and its leases lapse at the TTL. Any expiry-driven reaper (stage 2) must exclude seats on a provider with no heartbeat seam, or the reaper reclaims live work. | `internal/bootstrap/packs/core/overlay/per-provider/*`; `internal/hooks/config/README.md` (event table); `cmd/gc/cmd_hook_heartbeat.go`; `cmd/gc/cmd_mail.go`; gascity PR #128 (codex round-3 P1); ga-56nq1a |
