...
| SESSION-RUNTIME-010 | Current-only writer identity set | Liveness writers (the ga-56nq1a lease heartbeat) authorize per-bead writes only under identities the session answers to NOW — session bead ID, raw session_name, configured_named_identity, current alias — via `CurrentAssigneeIdentities`, which excludes `alias_history`. Readers deciding whether to strip work keep the full historical set (`AssigneeIdentities`): a reader missing a prior alias resets live work, while a writer matching through a reused prior alias keeps a successor session's claims looking alive after the successor dies. The two sets are a deliberate pair; restoring history to the writer set reintroduces cross-session heartbeats. | `internal/session/assignee_identities_test.go` `TestCurrentAssigneeIdentitiesExcludesAliasHistory`; `cmd/gc/cmd_hook_heartbeat.go`; gascity PR #128 (codex round-2 P1); ga-56nq1a |
