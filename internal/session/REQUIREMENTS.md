...
| ID | Scenario | Required behavior | Evidence |
|---|---|---|---|
| SESSION-TERM-001 | Every ending leaves a record | Every path that stops a session goes through `runtime.StopRecorded`, which takes a MANDATORY `runtime.Termination` whose `Kind` is in a closed set. A caller that cannot classify its ending passes `KindUnclassified`, which is counted and budgeted; it may not pass nothing. An invalid or empty kind is COERCED to unclassified rather than rejected, because refusing would mean refusing to stop a session over bookkeeping. A direct `Provider.Stop` outside the seam is a fence violation. | `internal/runtime/termination.go`; `internal/runtime/termination_test.go`; `scripts/check-termination-seam.sh` (fence + `--self-test`) |
| SESSION-TERM-002 | The stop never waits on the record, and never fails because of it | Sinks are attempted BEFORE the stop so the record survives a stop that then wedges, and the stop proceeds regardless of whether a sink failed, panicked, or hung: the whole sink pass is bounded by `TerminationSinkBudget` (5s), past which the stop proceeds and the timeout is reported AS the sink error. `Manager` lifecycle methods return only the STOP's error — a sink failure is logged, never returned — because Suspend and CloseDetailed branch on it to decide whether to persist the suspended/closed state, and returning it left a dead runtime behind a live bead. | `internal/runtime/termination.go` (`StopRecordedDetailed`); `internal/session/manager.go` (`stopRecorded`); `internal/session/manager_termination_test.go` (`TestASinkFailureDoesNotAbortTheLifecycleTransition`, `TestARealStopFailureStillFails`, `TestManagerStopSurvivesASickSink`) |
| SESSION-TERM-003 | Two sinks, two failure domains | The authoritative record is `termination.{kind,actor,reason,at,requested_at}` written to the session bead through `ApplyPatch`; a `session.terminated` event corroborates it. They fail independently on purpose: the bead write rides Dolt and can fail during the incidents that produce force-exits, the event is a local append that almost never does. Bead history is the source of record; where a bead record is missing but an event exists, the event FILLS the hole and the week is FLAGGED, never silently patched. The keys stay on the VERSIONED bead and must not move to the dolt-ignored liveness table. | `internal/session/termination_sink.go`; `internal/runtime/terminationevents/sink.go`; `internal/session/termination_sink_test.go` |
| SESSION-TERM-004 | The kind belongs to the caller, not the mechanism | One `handle.Kill` serves `gc session kill`, a drain force-stop, a city stop and a remote handoff, so intent is passed in (`KillWithTermination` / the optional `worker.TerminationIntentKiller`) and a bare `Kill` records the honest unclassified. A kind fixed at a shared mechanism mislabels every caller its author did not picture: while it was, every drain recorded as `operator-kill` and `drain-timeout` could never appear at all. | `internal/worker/handle_lifecycle.go`; `cmd/gc/worker_handle.go` (`workerKillSessionTargetWithTermination`); `cmd/gc/session_wake.go` (`verifiedStop`); `internal/session/manager_termination_test.go` |
| SESSION-TERM-005 | A kind stated by one path, consumed by another | A self-handoff does not stop its own runtime: `gc handoff` stamps `termination.intent` / `termination.intent_at` on the session bead and the reconciler's restart-consume records THAT kind, with `RequestedAt` from the stamp so `At - RequestedAt` is real. The intent is CONSUMED AT MOST ONCE, cleared whether or not it was used, because a handoff whose restart never arrives would otherwise relabel the next unrelated ending. | `cmd/gc/cmd_handoff.go` (`recordHandoffTerminationIntent`); `cmd/gc/session_reconciler.go` (`restartRequestTermination`); `internal/session/termination_sink.go` (`TerminationIntentPatch`) |
| SESSION-TERM-006 | Every kind must have a producer, and every seam call a sink | A kind that no production code constructs reads ZERO forever on a table that looks fully populated, and a call routed through the seam with an EMPTY sink list records nowhere while the fence reads clean. Both are checked mechanically. Kinds whose producer legitimately arrives with a later slice are pre-registered and the guard fails in BOTH directions. `drain-timeout` means the seat was asked; where nothing asks (a named seat, whose ack the reconciler publishes on its behalf) the kind is `drain-silent`. | `internal/runtime/termination_producers_test.go`; `cmd/gc/termination_sink_wiring_test.go`; `cmd/gc/session_wake.go` (`drainAckCameFromTheAgent`) |
| SESSION-TERM-007 | The record is an ATTEMPT, not a confirmed outcome | `termination.*` asserts "an ending of this kind was attempted at this moment", never "the session is gone". Inside the reconciler's unavailable-probe window a record legitimately disagrees with a session that turns out to be running, and a reader must not treat the record alone as proof of death — drain-completion state is what says a drain concluded. Retries of one ending write N history rows; readers deduplicate on `(SessionID, EventID)`. `EventID` is minted once per ENDING: a caller that can retry (the drain) mints it with `NewTerminationEventID` beside its stable start and passes it on every attempt, and a synchronous caller leaves it empty for the seam to mint. No attribute key works: the earlier `(SessionID, Kind, RequestedAt)` collapsed real twins, because two synchronous suspends, closes, or city stops of one session both carry a zero `RequestedAt`. | `internal/runtime/termination.go` (type doc, `EventID`); `internal/events/session_terminated_payload.go`; `cmd/gc/session_wake.go`; ga-ksac39 |
