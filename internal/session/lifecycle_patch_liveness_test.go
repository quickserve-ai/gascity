...
func TestHotPatchBuildersEmitOnlyLivenessKeys(t *testing.T) {
	...
	builders := map[string]MetadataPatch{
		...
		"ClearWakeBlockersPatch":      ClearWakeBlockersPatch(StateSuspended, string(SleepReasonUserHold), now),
	...
	for name, patch := range map[string]MetadataPatch{
		...
		"AcknowledgeDrainPatch/fresh-wake": AcknowledgeDrainPatch(now, true),
