...
|---|---|---|---|
| RUNTIME-PLAN-003 | Pack-declared runtimes | `[runtimes.<name>]` in pack.toml (command, protocol version) registers a runtime during city composition; collisions with builtin names are errors; a doctor check verifies install + handshake; `gc runtime check <name>` resolves pack-declared names. | `ga-h504e5` |
