...
func TestColdStartServerArgvAndEnvCarryNoSecrets(t *testing.T) {
	...
	if strings.Contains(string(argv), "AZURE_OPENAI_API_KEY") {
		t.Fatalf("tmux server argv names a secret session env key; it was founded by an -e new-session: %s", string(argv))
	}
