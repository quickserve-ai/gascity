...
//
// Both remotes are https, and sync pushes a non-file:// remote only when
// GC_DOLT_REMOTE_<DB> names it (#6010: with no override such a database is
// skipped as ambiguous). The list names both, in the pre-fix pick order, so
// every assertion below still exercises the per-remote loop.
func runMultiRemoteSync(t *testing.T, binDir string) (log, out, freshnessDir string) {
	...
	cmd.Env = append(syncFilteredEnv(),
		...
		"GC_DOLT_USER=root",
		"GC_DOLT_PASSWORD=",
		"GC_DOLT_REMOTE_APP=origin,probe",
	)
...
// originMirrorsReport is the backups.origin_mirrors plane of `gc dolt health
// --json`, where the per-remote mirror verdict is reported. The dolt_* fields
// beside it date the LOCAL backup remotes by their manifests (#5329) and say
// nothing about off-box mirrors.
type originMirrorsReport struct {
	State string `json:"state"`
	Stale bool   `json:"stale"`
}

func originMirrors(t *testing.T, out string) originMirrorsReport {
	t.Helper()
	var report struct {
		Backups struct {
			OriginMirrors originMirrorsReport `json:"origin_mirrors"`
		} `json:"backups"`
	}
	if err := json.Unmarshal([]byte(out), &report); err != nil {
		t.Fatalf("health --json is not valid JSON: %v\n%s", err, out)
	}
	return report.Backups.OriginMirrors
}
...
func TestHealthRequiresAStampForEveryConfiguredRemote(t *testing.T) {
	...
	if m := originMirrors(t, out); m.State == "ok" || !m.Stale {
