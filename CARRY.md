...
| Claude-account env residuals + pre-spawn declaration guard (ga-ai7gz2, ga-xd3bjx; fork PRs #40+#48) | bb981d2f3, 47c763d4d, 9f47485ab, 7b63e700d | Managed sessions never inherit the controller's ambient claude-account env: template-resolution guard refuses undeclared claude seats (#40); unresolved-provider resume gets an env-only reset-baseline override with a permissive-lookPath retry so a PATH-missing-but-declared provider keeps its env; respawn re-applies cfg.Env incl. env -u unsets; direct-create and resume merge Workspace.Env per template step 10; doctor check `claude-account-declaration` flags undeclared claude seats against the supervisor service env pre-spawn (#48). | NOT headed upstream — upstream deliberately passes CLAUDE_CONFIG_DIR through. At the next rebase onto a tip containing upstream #5061 (cab9da1c8, reshaped respawnAgent): re-express the respawn half on #5061's markSessionEnvRemoved/durableWithholdKeys; the guard, resume-override, Workspace.Env-merge and doctor halves carry until upstream adopts account-declaration semantics. |
