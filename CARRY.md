...
| Native Dolt metadata merge is a compare-and-swap (gc-sey7; syl qc-04ff7.109.12; upstream #6222) | 0f191c7f2 (= A3Ackerman fix/native-store-metadata-checked-merge 6639fb216, cherry-pick -x, clean apply on cc6dcb7b1; carry(test) companions add the storage spy's `updateIssueChecked` hook that the picked tests drive — upstream's spy already has it — and, for the picked real-store proofs, expose the test dolt server's port (`startTestDoltServerPort`) and use carry's `rawDBGetter` name) | `NativeDoltStore.SetMetadataBatch` (and `SetMetadata`) writes the merged map back with `UpdateIssueChecked` carrying the read's `RowVersion` as `ExpectedVersion`, and re-runs the whole read-merge-write on `ErrVersionMismatch` (`retryOnNativeDoltMergeRace`, the serialization-conflict retry's budget), so an `Update` that commits between the read and the write is merged onto instead of replaced by the stale map; the standalone update's audit event is kept (no transaction). The lost update that re-fenced a review root's entry step with a bare route on westeros; protects every metadata stamp that races an update on the same bead. Also carried: upstream #5099's `error 1205` classifier line (own commit) — the library's own retry covers 1205 for permanent rows but its wisp path (`dolt/wisps.go:212`) has no retry of its own, so a 1205 on a wisp reached this classifier and failed the stamp; regression `native_dolt_store_lock_wait_retry_test.go`. And the real-store proof (pick of upstream 6d0988ee0): a test-only `afterMetadataMergeRead` hook plus `native_dolt_store_metadata_merge_integration_test.go`, green here on the embedded backend — a competing write on other keys between the read and the checked write is merged onto, one event recorded. The rollback proof in the same file runs against a throwaway dolt sql-server (`startTestDoltServerPort`), fails closed, and requires the table-not-found diagnostic. Not coverage: the pre-existing `TestNativeDoltStoreEventsIDDefaultRepair` still skips on the embedded backend (no raw DB) and its 'want failure' expectation predates the library's explicit event ids — its own carry(test) fix follows. | Drop when the gastownhall PR for #6222 merges (patch-id equal at the next rebase). |
