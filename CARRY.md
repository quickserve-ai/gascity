...
| Reconciler ack ownership — the reconciler's drain ack under its own keys (gc-hzj5; Cherub strict-review finding on #6178) | 74baf90d1 (= upstream PR branch 112dab235, cherry-pick -x, clean apply) | The reconciler publishes its ack as source/reason/generation only and never writes GC_DRAIN_ACK (the agent's key); isDrainAcked reads either shape; the hold's tracked and recovered cancels clear only the reconciler's keys and remove GC_DRAIN alone, so an agent ack landing at any interleaving survives and outranks the hold (closes the check-then-act race found on 1d26598da). Every other clear keeps its full effect. | Absorbed with e6c49e254 when the gastownhall PR for #6173 (#6178) merges — drop at the next rebase. |
