...
| The supervisor's per-city loader applies the daemon feature flags (gc-irxm; syl qc-04ff7.109; upstream #6236) | 3a145e88a (= A3Ackerman fix/supervisor-loader-feature-flags 1dc450087, cherry-pick -x, resolved on cc6dcb7b1 for the trace-line hunk: carry lacks the nativeStepTopologyPrepared lines above it) | `loadCityConfigWithBuiltinPacks` calls `applyFeatureFlags(cfg)` right after `config.LoadWithIncludes`, as `loadCityConfigFS` does for the CLI; before, `molecule.graphApplyEnabled` kept its zero value in the supervisor process until an API request or a reload applied it, so a supervisor-run city minted every graph.v2 order on the sequential path (the 62-of-62 sequential controller mints on westeros) while `gc order run` minted atomically. Plus one `GC_SLING_TRACE` line at `molecule.Instantiate`'s constructor choice (`graph-apply-enabled`, `defer-assignees`). Decided by execution: the test fails on cc6dcb7b1 and on stock ae5163dd3 alike. | Drop when the gastownhall PR for #6236 merges (patch-id differs by the resolved hunk; verify by `TestLoadCityConfigWithBuiltinPacksAppliesFeatureFlags`). |
