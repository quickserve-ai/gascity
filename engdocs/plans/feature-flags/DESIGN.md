...
var specs = []Spec{
	{
		...
		EnvOverride:  "GC_BEADS_CONDITIONAL_WRITES", // named consumer: hosted cities with
		EnvSemantics: EnvOverrides,                  // baked/immutable config (crucible model)
...
- Lift the prohibition only when ALL hold:
  ...
  (3) >= 1 week soak on maintainer-city at `auto` with zero degraded events
...
| Y-12 | LOW | CAS env override has no demonstrated consumer (process-latched ⇒ restart either way) | A | Kept with its consumer named in the Spec rationale: hosted cities with baked/immutable config (the crucible deployment model), where env is the only injectable surface (§5) |
...
3. **Deployed-topology test ownership:** the sqlite `ConditionalWriter` integration test must run against the `deploy/sqlite-b36-probe-attribution` store shape maintainer-city actually runs — port that harness into main's fixtures, or stage a deploy-branch test as the stage-3 gate? (Owner TBD.)
