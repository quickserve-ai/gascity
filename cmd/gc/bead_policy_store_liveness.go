...
//
// The predicate is a deliberate SUPERSET of "is a session bead": the write
// splitter routes by key, not by bead type, so the metadata clause also admits
// any bead whose committed metadata names a moved key or a fence marker. A
// degraded or transactional write commits the moved keys together with one
// fence marker per key, so a bead that ever took one stays in the candidate set
// from then on.
//
