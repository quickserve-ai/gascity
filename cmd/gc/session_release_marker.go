...
func publishDeferredReleaseObligation(store beads.Store, sessionID string, obligation sessionReleaseObligation) deferredReleaseResult {
	...
	writer, conditional := beads.ConditionalWriterFor(store)
	if !conditional {
...
func readSessionReleaseObligation(store beads.Store, sessionID string) (sessionReleaseObligation, string, error) {
	...
	raw := strings.TrimSpace(bead.Metadata[beadmeta.ReleaseDeferredMetadataKey])
	if raw == "" {
...
func deferMissingConfigWorkRelease(store beads.Store, sessionID string, sessionBead beads.Bead, captureOK bool, reason string, rigStoresKnown bool, rec events.Recorder, now time.Time, stderr io.Writer) deferredReleaseResult {
	...
	case !result.Persisted:
		fmt.Fprintf(stderr, "gc session close: WARNING: could not record the deferred work release for session %s (%v). "+ //nolint:errcheck // best-effort stderr
			...
			"re-run this close once the city config loads.\n", sessionID, result.Err)
	case result.Published && reported.Capture == releaseCaptureIDOnly:
		fmt.Fprintf(stderr, "gc session close: recorded a deferred work release for session %s (generation %d), "+ //nolint:errcheck // best-effort stderr
			"but the session bead could not be read, so only its ID was captured — the withheld work held under a "+
			"NAME is not listed. Re-run this close once the city config loads.\n", sessionID, reported.Generation)
	case result.Published:
		fmt.Fprintf(stderr, "gc session close: recorded a deferred work release for session %s (generation %d, %d identifiers). "+ //nolint:errcheck // best-effort stderr
			"The withheld work is not released yet; re-run this close once the city config loads.\n",
			sessionID, reported.Generation, len(reported.Identities))
	default:
...
func formatDeferredReleaseMessage(sessionID string, obligation sessionReleaseObligation, result deferredReleaseResult) string {
	if !result.Persisted {
		return fmt.Sprintf("session %s closed with its work release withheld (%s), and the deferred release could NOT be recorded: %v",
	...
	return fmt.Sprintf("session %s closed with its work release withheld (%s); deferred release recorded at generation %d, capture %s, %d identifiers, scope %s",
