...
// TestDoltDriftCheckProxiedCityHasNoPortToResolve: a proxied Dolt (bd-owned,
// upstream #6273's default) has no managed city port, so the empty port is
// not a degraded probe and the check must not warn that its analysis was
// skipped; the topology acceptance suite wants doctor green on that city.
func TestDoltDriftCheckProxiedCityHasNoPortToResolve(t *testing.T) {
	cityDir, rigDir, _, cfg := managedCityDriftFixture(t, "proxied")
	for _, scope := range []string{cityDir, rigDir} {
		if _, err := contract.EnsureCanonicalMetadata(fsys.OSFS{}, filepath.Join(scope, ".beads", "metadata.json"), contract.MetadataState{
			Database:     "dolt",
			Backend:      "dolt",
	...
	if doctor.ManagedLocalDoltChecksApplicableForConfig(cityDir, cfg, nil) {
		t.Skip("fixture still reads as managed-local; the proxied shape is not what this test exercises")
	}
	r := newDoltDriftCheck(cityDir, cfg).Run(&doctor.CheckContext{CityPath: cityDir})
	if r.Status == doctor.StatusWarning && strings.Contains(r.Message, "port unresolvable") {
		t.Fatalf("a proxied city has no managed port to resolve; got %q details=%v", r.Message, r.Details)
	}
}
