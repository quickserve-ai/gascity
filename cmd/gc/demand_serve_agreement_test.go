...
// run_target is consulted only when there is no canonical route — and drops any
// row parked under a hold:* label (ga-uica16). Mirroring those two jq clauses
...
func assertLegacyTierFilterUnchanged(t *testing.T, query string) {
	...
	const wantFilter = `jq '[.[] | select((.metadata["gc.routed_to"] // "") == "") | select(([.labels[]? | select(startswith("hold:"))] | length) == 0)] | .[:1]'`
