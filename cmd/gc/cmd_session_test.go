...
// The ga-e4jb refusal must not pre-empt #3884 Fix B: when the single-session
// template backs a named session of the SAME name, gc session new materializes
// that configured identity instead of minting a shadow, so there is nothing to
// refuse.
func TestCmdSessionNew_SameNameNamedSessionMaterializesInsteadOfRefusing(t *testing.T) {
	t.Setenv("GC_BEADS", "file")
	t.Setenv("GC_SESSION", "fake")
...
`, rigRoot))
	...
	var stdout, stderr bytes.Buffer
	if code := cmdSessionNew([]string{"demo/helper"}, "", "", "", true, false, 0, &stdout, &stderr); code != 0 {
		t.Fatalf("cmdSessionNew(same-name named template) = %d, want 0; stderr=%s", code, stderr.String())
	}
	b := onlySessionBead(t, cityDir)
	if got := b.Metadata["session_origin"]; got != "named" {
		t.Errorf("session_origin = %q, want named", got)
	}
	if got := b.Metadata[session.NamedSessionIdentityMetadata]; got != "demo/helper" {
		t.Errorf("%s = %q, want demo/helper", session.NamedSessionIdentityMetadata, got)
	}
}
