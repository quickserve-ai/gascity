...
// hookWorkQueryTimeout caps the work-query subprocess. Restored to 30s after
// ga-j2bv deployed the indexed gc.routed_to metadata lookup: six serial
// no-match probes measured 7.86s total, and eight concurrent probes completed
// within 2.51s each. The former 90s mitigation protected the unindexed
// 10-23s-per-probe path but now delays recovery from a genuinely wedged store.
// Keep this variable package-scoped so timeout behavior remains testable.
