...
func (cr *CityRuntime) beadReconcileTick(ctx context.Context, result DesiredStateResult, sessionBeads *sessionBeadSnapshot, trace *sessionReconcilerTraceCycle, bootReconcile bool) {
	...
	if poolDesired == nil {
		phaseStart = time.Now()
		poolWorkBeads := filterAssignedWorkBeadsForPoolDemand(cr.cfg, cr.cityPath, sessionBeads.OpenInfos(), assignedWorkBeads, assignedWorkStoreRefs, result.PoolWakeReadiness)
...
func (cr *CityRuntime) controlDispatcherTick(ctx context.Context) {
	...
	poolWorkBeads := filterAssignedWorkBeadsForPoolDemand(filteredCfg, cr.cityPath, openInfos, wfcResult.AssignedWorkBeads, wfcResult.AssignedWorkStoreRefs, wfcResult.PoolWakeReadiness)
	poolDesired := retainScaleCheckPartialPoolDesired(
		filteredCfg,
...
func (cr *CityRuntime) loadDemandSnapshot(
	...
	if cr.shouldRefreshDemandSnapshot(trigger, configChanged, sessionFingerprint) {
		...
		poolWorkBeads := filterAssignedWorkBeadsForPoolDemand(cr.cfg, cr.cityPath, openSessionInfos, result.AssignedWorkBeads, result.AssignedWorkStoreRefs, result.PoolWakeReadiness)
