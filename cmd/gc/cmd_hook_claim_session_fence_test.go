...
func TestWriteHookClaimDrainStaleSessionWithDrainAck(t *testing.T) {
	...
	var stdout, stderr bytes.Buffer
	code := writeHookClaimDrain(hookClaimLabel, hookClaimReasonStaleSession, 0, true, true, fakeAck, &stdout, &stderr)
...
func TestWriteHookClaimDrainDoesNotAckWhenNotRequested(t *testing.T) {
	...
	var stdout, stderr bytes.Buffer
	code := writeHookClaimDrain(hookClaimLabel, hookClaimReasonStaleSession, 0, true, false, fakeAck, &stdout, &stderr)
	if code != 1 {
