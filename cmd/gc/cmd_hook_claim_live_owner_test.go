...
func TestHookClaimIdentityPatchPreservesDisplacedSession(t *testing.T) {
	...
	ops := hookClaimOps{ResolveWorkBranch: func(hookClaimWorkTree) string { return "" }}
	...
	// Re-adoption of the claimer's own bead. The bead already carries
	// gc.claimed_at, as any bead this session claimed does: upstream stamps that
	// key write-once when it is absent, which is not a session-identity change.
	patch = hookClaimIdentityPatch(beads.Bead{
		ID:       "qc-z",
		Metadata: map[string]string{"gc.session_id": "ga-me", "gc.claimed_at": "2026-09-03T00:00:00Z"},
	}, opts, ops, "/tmp/work")
	if _, ok := patch["gc.prev_session_id"]; ok {
		t.Errorf("re-adoption of own bead stamped prev session id: %v", patch)
	}
	if len(patch) != 0 {
...
// liveOwnerReclaimSpy records the scoped stale-lease reclaims (#6402) and the
// reclaimed-stale events the claim path would emit.
type liveOwnerReclaimSpy struct {
	reclaimed []string
	emitted   []string
}

func (s *liveOwnerReclaimSpy) wire(ops *hookClaimOps, opts *hookClaimOptions) {
	opts.AutoReclaimStaleClaims = true
	ops.ReclaimStale = func(_ context.Context, _ string, _ []string, beadID string) (bool, string, error) {
		s.reclaimed = append(s.reclaimed, beadID)
		return true, "worker-old", nil
	}
	ops.EmitHookClaimReclaimedStale = func(beadID, previousOwner, newAssignee string) {
		s.emitted = append(s.emitted, beadID+"|"+previousOwner+"|"+newAssignee)
	}
}

// The stale-assignment reclaim (#6402) is a mutation — bd reverts the lease the
// moment it judges it stale — so the live-owner guard must run BEFORE it. bd's
// lease TTL says nothing about whether the session recorded in gc.session_id is
// still running: a lapsed lease held by a live session must be declined, never
// reclaimed and taken over.
func TestHookClaimLiveOwnerGuardGatesStaleReclaim(t *testing.T) {
	for _, tc := range []struct {
		name    string
		verdict hookOwnerSessionVerdict
		reason  string
	}{
		{name: "recorded session live", verdict: hookOwnerSessionLive, reason: `session state "active"`},
		{name: "recorded session liveness unknown fails closed", verdict: hookOwnerSessionUnknown, reason: "opening session store: boom"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			spy := &liveOwnerClaimSpy{}
			reclaim := &liveOwnerReclaimSpy{}
			probe := func(owner string) (hookOwnerSessionVerdict, string) {
				if owner != "ga-other" {
					t.Errorf("probe asked about %q, want ga-other", owner)
				}
				return tc.verdict, tc.reason
			}
			ops, opts := liveOwnerGuardOpsOpts(
				`[{"id":"qc-lease","status":"in_progress","assignee":"worker-old","metadata":{"gc.routed_to":"worker","gc.session_id":"ga-other"}}]`,
				spy, probe,
			)
			reclaim.wire(&ops, &opts)

			var stdout, stderr bytes.Buffer
			doHookClaim("bd ready --json", "/tmp/work", opts, ops, &stdout, &stderr)

			if len(reclaim.reclaimed) != 0 {
				t.Fatalf("reclaims = %v, want none — a live-owned lease must not be reverted", reclaim.reclaimed)
			}
			if len(spy.ids) != 0 {
				t.Fatalf("claim mutations = %v, want none", spy.ids)
			}
			if len(reclaim.emitted) != 0 {
				t.Fatalf("reclaimed-stale events = %v, want none", reclaim.emitted)
			}
			if !strings.Contains(stderr.String(), "declined-live-owner: 1") {
				t.Errorf("stderr = %q, want a loud declined-live-owner summary", stderr.String())
			}
...
// A stale lease whose recorded session is gone is the case #6402 exists for:
// the reclaim, the same-cycle claim and the reclaimed-stale event all proceed.
func TestHookClaimReclaimsStaleLeaseWhenRecordedSessionIsGone(t *testing.T) {
	spy := &liveOwnerClaimSpy{}
	reclaim := &liveOwnerReclaimSpy{}
	probe := func(string) (hookOwnerSessionVerdict, string) {
		return hookOwnerSessionGone, "session bead is closed"
	...
	ops, opts := liveOwnerGuardOpsOpts(
		`[{"id":"qc-lease","status":"in_progress","assignee":"worker-old","metadata":{"gc.routed_to":"worker","gc.session_id":"ga-dead"}}]`,
		spy, probe,
	)
	reclaim.wire(&ops, &opts)
	...
	if len(reclaim.reclaimed) != 1 || reclaim.reclaimed[0] != "qc-lease" {
		t.Fatalf("reclaims = %v, want exactly [qc-lease]", reclaim.reclaimed)
	}
	if len(spy.ids) != 1 || spy.ids[0] != "qc-lease" {
		t.Fatalf("claim mutations = %v, want qc-lease claimed in the same cycle", spy.ids)
	}
	if want := "qc-lease|worker-old|worker-1"; len(reclaim.emitted) != 1 || reclaim.emitted[0] != want {
		t.Fatalf("reclaimed-stale events = %v, want [%q]", reclaim.emitted, want)
	}
	if strings.Contains(stderr.String(), "declined-live-owner") {
		t.Errorf("stderr = %q, want no decline for a gone owner", stderr.String())
	}
}
