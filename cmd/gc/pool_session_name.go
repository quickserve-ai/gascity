...
// that were closed.
func GCSweepSessionBeads(store beads.Store, rigStores map[string]beads.Store, cfg *config.City, sessionInfos []session.Info) []string {
	var closed []string
	for _, info := range sessionInfos {
		if info.Closed {
			continue
		}
		if !closeSessionInfoIfUnassigned(store, rigStores, cfg, info, "gc_swept", time.Now().UTC(), nil) {
			continue
		}
