...
func poolDesiredForStore(t *testing.T, store beads.Store, sessions ...beads.Bead) []PoolDesiredState {
	...
	poolWork := filterAssignedWorkBeadsForPoolDemand(cfg, "", infos, work, workRefs, wakeReady)
...
func TestPoolWakeReadinessFailsOpenOnReadError(t *testing.T) {
	...
	poolWork := filterAssignedWorkBeadsForPoolDemand(cfg, "", nil, work, workRefs, wakeReady)
...
// blocking dependency at all. The row carries an ELAPSED defer_until, which is
// the variant that survives #5094's IsDeferred gate.
//
...
// this variant nor the indefinite one — nor does `bd ready --include-deferred`,
// whose flag relaxes only a future timestamp. Both are unclaimable; the demand
// side must agree.
//
// CARRY NOTE: upstream drives this fixture for BOTH variants the issue names,
// the second being the reporter's status=deferred/defer_until-absent row set via
// Bead.IndefinitelyDeferred. That field does not exist on this branch (see
// beads.CarriesDeferral), so only the elapsed variant is exercised here — which
// is the variant that needed this change. The indefinite case returns with the
// field at the next upstream resync.
func deferredRoutedDemandFixture(t *testing.T) (beads.Store, string) {
	t.Helper()
	mem := beads.NewMemStore()
...
// reproduction, driven through the builder.
//
...
func TestBuildDesiredStateWithholdsPoolSessionForDeferredRoutedWork(t *testing.T) {
	t.Run("defer_until elapsed", func(t *testing.T) {
		store, rowID := deferredRoutedDemandFixture(t)
		// The frontier's own answer, recorded so the test says WHY the variant
		// is withheld rather than just that it is.
