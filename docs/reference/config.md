...
| `run_stale_after` | string |  |  | RunStaleAfter overrides how long the order's formula runs may stay open before they count as stale, when a stale run whose claiming session of this city has ended is closed so the order can fire again. Go duration string; default 6h. Lets a deployment raise it for a scanned shared-pack order whose runs legitimately take longer, without editing the pack source. |
